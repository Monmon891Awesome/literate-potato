...
def nfa_to_dfa(nfa):
    ...
    def collect_symbols(state, visited=None):
        if visited is None:
            visited = set()
        ...
        for eps_state in state.epsilon_transitions:
            collect_symbols(eps_state, visited)
    ...
    dfa_states[frozenset(start_closure)] = 0  # Start state is 0

    # Check if the start state is also a final state
    is_final = any(state.is_final for state in start_closure)
    dfa_final_states = {0} if is_final else set()
    ...
    while queue:
        ...
        for symbol in alphabet:
            ...
            next_states = frozenset(epsilon_states)
            ...
            if next_states not in dfa_states:
                dfa_states[next_states] = len(dfa_states)
                queue.append(next_states)

                # Check if it's a final state
                if any(state.is_final for state in next_states):
                    dfa_final_states.add(dfa_states[next_states])

            # Add transition to DFA
            dfa_transitions[(current_dfa_state, symbol)] = dfa_states[next_states]
...
def visualize_dfa(dfa, highlight_states=None, highlight_transitions=None):
    ...
    # Draw the graph
    node_colors = ['lightgreen' if node in highlight_states else ('lightblue' if G.nodes[node]['is_final'] else 'white')
                   for node in G.nodes]
...
def display_regex_conversion(regex, input_string=None):
    ...
    with st.expander("Pushdown Automaton (PDA) Representation"):
        st.markdown("### States")
        st.write(f"States: {', '.join([f'q{s}' for s in pda['states']])}")
        st.write(f"Start State: q{pda['start_state']}")
        st.write(f"Final States: {', '.join([f'q{s}' for s in pda['final_states']])}")

        st.markdown("### Alphabets")
        st.write(f"Input Alphabet: {', '.join(pda['input_alphabet'])}")
        st.write(f"Stack Alphabet: {', '.join(pda['stack_alphabet'])}")

        st.markdown("### Transitions")
        for (state, symbol, stack_top), (next_state, new_stack) in pda['transitions'].items():
            new_stack_str = ''.join(new_stack) if new_stack else 'ε'
            st.write(f"δ(q{state}, {symbol if symbol != 'ε' else 'ε'}, {stack_top}) = (q{next_state}, {new_stack_str})")
...
st.markdown('<h2 class="sub-header">Enter a string to check its validity for displayed DFA</h2>',
            unsafe_allow_html=True)
